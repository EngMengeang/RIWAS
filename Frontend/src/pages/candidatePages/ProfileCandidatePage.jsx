...
const ProfileCandidate = () => {
        ...
        try {
          ...
          if (err?.response?.status === 404) {
            try {
              ...
              try { data = await profileAPI.get(userId); } catch { /* use create response */ }
            } catch (createErr) {
              throw new Error(
  ...
  const handleUserInfoChange = (e) => {
    ...
    if (name === "phone") setProfile((prev) => ({ ...prev, phonenumber: value }));
  };
  ...
  // ── Preview: navigate to ViewCVCandidate page ────────────────────────────────
  const handlePreview = (resume) => {
    navigate(`/my-cv/${resume.id}`, {
      state: {
        resumeUrl: getFileUrl(resume),
        fileName:  getFileName(resume),
      },
    });
  };
              ...
              <>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:gap-6 gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    {userInfo.createdAt && (
                      <p className="text-gray-400 text-sm">Created at {formatDate(userInfo.createdAt)}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      name="fullName" value={userInfo.fullName} disabled={!isEditing}
                      onChange={handleUserInfoChange} placeholder="Your full name"
                  ...
                  <div>
                    ...
                    <input
                      name="phone" value={userInfo.phone} disabled={!isEditing}
                      onChange={handleUserInfoChange} placeholder="+855 96 7817 889"
                      className="w-full mt-1 p-2 border rounded-md border-gray-300 disabled:bg-gray-100"
                    />
                  </div>
                  <div>
                ...
                {/* ── PROFILE DETAILS ── */}
                <h3 className="text-lg font-semibold mt-8 mb-4">Profile Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[
                    { label: "Headline",  name: "headline",   placeholder: "e.g. Senior Frontend Developer" },
                    { label: "Website",   name: "websiteUrl", placeholder: "https://yourwebsite.com" },
                    ...
                    { label: "Twitter",   name: "twitter",    placeholder: "https://twitter.com/username" },
                  ].map(({ label, name, placeholder }) => (
                    <div key={name}>
                      <label className="text-gray-600 text-sm">{label}</label>
                      <input
                        name={name} value={profile[name]} disabled={!isEditing}
                        onChange={handleProfileChange} placeholder={placeholder}
                ...
                <div className="mt-6">
                  ...
                  <textarea
                    name="bio" rows="4" disabled={!isEditing} value={profile.bio}
                    onChange={handleProfileChange} placeholder="Tell us about yourself…"
                    className="w-full mt-1 p-2 border rounded-md border-gray-300 disabled:bg-gray-50"
                  />
                </div>
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    name="about" rows="3" disabled={!isEditing} value={profile.about}
                    onChange={handleProfileChange} placeholder="More details about you…"
                    className="w-full mt-1 p-2 border rounded-md border-gray-300 disabled:bg-gray-50"
                  />
                </div>
                ...
                <div className="mt-6 flex flex-col md:flex-row gap-4">
                  <button
                    onClick={handleEditToggle} disabled={saving}
